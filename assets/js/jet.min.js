/*
 scoped css 连续刷新当前页面在组件中有bug
 onroute 添加 next url 参数
 j-dialog中图片不支持名称
 j-dialog中不支持scoped

Jet.router.route.open
*/
(function () {
  //*********定义全局变量 开始 *******************/
  ...
  function _load(name, call, ecall) {
    return _JT.ajax({
      ...
      success: function (result) {
        if(result.indexOf('src="http://tieba.baidu.com/_PXCK_')!==-1){
          _load(name, call, ecall);
        }else{
          call(result);
        }
      },
  ...
  function _checkDialogScoped(out,v){
    if(_canUse('jui')){
      out._JT_findClass('j-diaol')._JT_attr(_scoped_ele_id,v);
    }
  }
  ...
  function _ctStyle(attr, out,route) {
    ...
    if(route){
      ...
      // _checkDialogScoped(out,'RouterOut')
      s._JT_attr({
